...
CACHE_PATH = project_root / "storage" / "cache" / "brain_lint_manifest.json"
...
def get_wiki_files() -> List[Path]:
    """Get all markdown files in brain/distilled, sorted for stable output."""
    return sorted(BRAIN_DISTILLED_DIR.glob("*.md"), key=lambda path: path.name.lower())
...
def main() -> None:
    ...
    for record in file_records:
        ...
        if record["needs_llm"] and record["name"] in selected_names:
            # Sửa lỗi UnicodeEncodeError trên Windows Terminal bằng cách xóa ký tự Tiếng Việt khi in log
