...
SMART_ROUTER_URL = os.getenv("SMART_ROUTER_URL", "http://localhost:4000/v1/chat/completions")
SMART_ROUTER_KEY = os.getenv("SMARTPROXY_API_KEY", "sk-antigravity-secret")
...
# [NEW] Lộ trình dự phòng Layered Fallback (3 tầng)
# Chiến lược: Primary -> Tier Backup (cùng năng lực) -> Last Resort (ag/gemini-3-flash)
MODEL_FALLBACK_CHAINS: Dict[str, List[str]] = {
    # Nhóm Reasoning cao (@profiler, @auditor)
    ...
    # Nhóm Structure chặt (@designer, @evaluator)
    "groq/qwen/qwen3-32b": [
        "groq/qwen/qwen3-32b",
        "groq/llama-3.3-70b-versatile",
        "ag/gemini-3-flash",
    ...
    # Nhóm Coding chuyên biệt (@engineer)
    "qw/qwen3-coder-plus": [
        "qw/qwen3-coder-plus",
...
def call_worker(
    messages: List[Dict[str, str]],
    model: str = "main-engine",  # [FIX 1] Đổi thành model tiêu chuẩn của SmartProxy
