...
- `/audit-exam [file_đề]` — Kích hoạt `@auditor` đối soát ngược từng câu hỏi. Thứ tự ưu tiên: (1) `1-projects/[Project_Name]/EXAM_Context_*.md` → (2) `3-resources/wiki/synthesis/` các file khác → (3) `3-resources/raw/` chỉ khi distilled không đủ. KHÔNG dùng general knowledge. Nếu không tìm thấy nguồn ở cả 3 tầng → ghi rõ `[KHÔNG TÌM THẤY NGUỒN]`, tuyệt đối không reconstruct.
...
CHECKPOINT:
  ...
  output_file: "[đường dẫn file sẽ tạo — VD: 3-resources/wiki/synthesis/EXAM_Context_M2.md]"
