...
DOCX_ROOT = WORKSPACE_ROOT / "3-resources" / "raw" / "Tổng hợp đề kiểm tra LMS" / "Khoa học máy tính"
TEST_BANK_DIR = WORKSPACE_ROOT / "3-resources" / "test-bank"
ASSETS_DIR = WORKSPACE_ROOT / "3-resources" / "assets"

def get_level2(path_str):
    if "Python" in path_str: return "Python"
    if "Scratch Junior" in path_str: return "Scratch_Jr"
    if "Scratch" in path_str: return "Scratch"
    if "Tynker" in path_str: return "Tynker"
    return "KHMT_General"
...
def extract_image(rid, rid_map, zip_ref, output_prefix):
    ...
    asset_name = f"ASSET_KHMT_{output_prefix}_{found_rid}{ext}"
...
def create_atom_file(q, level2, exam_de, source_name):
    file_id = f"KHMT_{level2}_MCQ_{exam_de}_Q{q['num']}"
    ...
    content.append("\n## 🔗 Liên kết tư duy")
    content.append(f"- [Rule 14] Đã mở file nguồn `.../KHMT/.../{source_name}` và xác nhận fact.")
    
    filepath.write_text("\n".join([c for c in content if c or c==""]), encoding="utf-8")

def main():
    count = 0
    # Walk through ALL subfolders of KHMT
    for root, dirs, files in os.walk(DOCX_ROOT):
        # Skip AI subfolders if they are already done (optional, but safer to let them re-run if needed)
        if "AI THCS" in root or "AI Tiểu học" in root: continue
        ...
        for f in files:
            if f.endswith(".docx") and not f.startswith("~$"):
                ...
                print(f"Extracting KHMT: {f} -> {level2}_{exam_de}")
    ...
    print(f"Total KHMT questions extracted: {count}")
