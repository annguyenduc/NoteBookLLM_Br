...
def get_level2(path_or_file):
    if "Python" in path_or_file: return "Python"
    if "Scratch 1+2" in path_or_file: return "Scratch"
    if "Scratch Junior" in path_or_file: return "Scratch_Jr"
    if "Tynker" in path_or_file: return "Tynker"
    return "KHMT_General"
...
def create_atom_file(q, level2, exam_de, source_name):
    ...
    content_lines.append(f"- [Rule 14] Đã mở file nguồn `3-resources/raw/Tổng hợp đề kiểm tra LMS/Khoa học máy tính/.../{source_name}` và xác nhận fact.")
...
def main():
    ...
    count = 0
    subfolders = ["Python", "Scratch 1+2", "Scratch Junior", "Tynker"]
