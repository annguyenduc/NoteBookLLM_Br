...
Lập trình một **hệ thống quiz kiến thức 5 câu**: lưu câu hỏi và đáp án vào List, lần lượt đọc câu hỏi bằng TTS, nhận câu trả lời từ người dùng, kiểm tra đúng/sai và ghi điểm. Cuối cùng một luồng tổng kết được kích hoạt qua Broadcast, dịch thông báo kết quả sang tiếng Anh bằng Translate và đọc to bằng TTS.
...
- **1 luồng tổng kết cuối** nhận Broadcast để hiển thị kết quả; có thể tách thành nhân vật thứ 2 hoặc giữ trên cùng nhân vật chính
...
- Sau 5 câu: **Broadcast** kích hoạt luồng tổng kết cuối
- Luồng tổng kết: tạo câu nhận xét tiếng Việt theo mức điểm → **Translate** sang Anh → **TTS** đọc cả 2

### 3. Sản phẩm yêu cầu bàn giao

- **File code:** `.sb3` đặt tên `[Chi nhánh]-[Họ tên]-AI THCS-4.sb3`
- **Ảnh PNG** chụp toàn bộ khối lệnh chuỗi hỏi đáp + luồng tổng kết Broadcast
...
| **4** | Broadcast & Tổng kết | Broadcast hoạt động sau câu 5; luồng tổng kết chạy đúng; tổng kết có Translate + TTS cả 2 ngôn ngữ | **25đ** |
...
- **Nhân vật giáo viên ảo**; phần "Học lại" có thể là **1 nhân vật nút riêng** hoặc **phím R** nếu muốn giữ code gợi ý gọn hơn
...
[Khi nhận "Bai 4" - Kết thúc]
  ...
  Show [nút Học lại] hoặc hiển thị thông báo nhấn R để học lại
...
Lập trình trò chơi tương tác AI: nhân vật "cái rổ" bám theo **điểm khuôn mặt người dùng** bằng Face Sensing, sau đó cố định lại trục Y để bắt các bóng rơi ngẫu nhiên từ trên xuống. Có đồng hồ đếm ngược 30 giây và bảng điểm hiển thị liên tục. Khi hết giờ, TTS đọc kết quả và luồng tổng kết hiển thị kết quả. Nhấn Space để chơi lại.

### 2. Yêu cầu kỹ thuật chi tiết

**Công cụ AI bắt buộc sử dụng:**
- **Face Sensing** (nhân vật rổ bám theo điểm khuôn mặt bằng block `go to face point`, sau đó cố định Y)
- Extension **Text to Speech** (đọc kết quả cuối trò chơi)

**Cấu tạo / Thành phần bắt buộc:**
- **Logic Rổ**: bám theo điểm khuôn mặt, cố định lại Y = -150
- **Logic Bóng**: xuất hiện X ngẫu nhiên (-200 đến 200), rơi từ Y=170 xuống Y=-180
- **Biến số** `score` và `time` (đếm ngược từ 30), hiển thị liên tục
- **Logic Tổng kết**: được kích hoạt sau Broadcast "gameOver"; khi triển khai hoàn chỉnh có thể đặt ở sprite riêng hoặc cùng sprite hiện tại
...
- **Ảnh PNG** chụp toàn bộ khối lệnh: logic Rổ (Face Sensing + Forever), logic Bóng (rơi + chạm + Broadcast), logic Tổng kết
...
| STT | Tiêu chí đánh giá | Mức độ mô tả chất lượng đạt yêu cầu | score tối đa |
| :---: | :--- | :--- | :---: |
| **1** | Face Sensing điều khiển Rổ | Rổ bám theo điểm khuôn mặt trong suốt trò chơi; Y được cố định -150 sau mỗi lần cập nhật để không bị nhảy | **25đ** |
...
- **1 luồng Người dẫn** để Say mô tả và TTS; có thể tách thành sprite riêng hoặc để ở sprite điều khiển chính
...
Ghi chú: file mẫu trong media đã tách các vai trò Mũ, Bông tai trái, Bông tai phải, Kính và Người dẫn; đoạn kịch bản trên rút gọn để giáo viên dễ nhìn thấy mẫu logic cần nhân bản giữa các phụ kiện.
```
...
**Cấu tạo / Thành phần bắt buộc:**
- **Teachable Machine** được huấn luyện đủ **≥ 3 nhãn** (3 thành viên) + 1 nhãn "Không ai" (background); URL model trong code gợi ý phải được thay bằng model thật trước khi chạy
- **1 List** `presentList` lưu tên các thành viên đã xuất hiện
- **Biến số** `time` đếm ngược từ 30, hiển thị liên tục
- **1 luồng tổng kết** sau Broadcast "timeUp"; có thể đặt ở nhân vật riêng hoặc ở script Broadcast của nhân vật chính

**Hoạt động / Tính năng bắt buộc:**
- Kết nối model Teachable Machine qua Extension ML trong Dancing with AI; thay URL placeholder bằng model thật của lớp trước khi demo
...
- **Ảnh PNG** chụp toàn bộ khối lệnh: luồng nhận diện chính (TM + List + TTS đọc tên) + luồng tổng kết (Translate + TTS 2 ngôn ngữ)
