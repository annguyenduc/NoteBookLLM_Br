...
(async () => {
  ...
  // Nạp khối lệnh turn video [on-flipped] cho Câu 17
  const xmlContent = `<xml xmlns="http://www.w3.org/1999/xhtml">
    <block type="poseFace_videoToggle" id="q17_video" x="50" y="50">
      <value name="VIDEO_STATE">
        <shadow type="poseFace_menu_VIDEO_STATE" id="video_val_17">
  ...
  console.log("Rendering Q17 block (turn video on-flipped)...");
  ...
  fs.writeFileSync(path.join(outDir, 'cau_17.xml'), xmlContent, 'utf8');

  await browser.close();
  console.log("Done rendering Q17 flipped!");
})();
