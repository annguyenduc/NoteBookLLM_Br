...
Hard boundaries:
- [MANDATORY source_id Lock]: `@scout` phải tra cứu tệp `1-projects/sources/source_registry.md` trước khi chạy. Nếu tài liệu nguồn nạp từ `00_Inbox/sources-pending/` chưa có ID, `@scout` phải tự động sinh `source_id` theo quy tắc `[LOẠI]_[VIẾT_TẮT]` và đăng ký vào bảng `source_registry.md` trước. Tuyệt đối không để trống hay ghi "NONE".
...
- file output mặc định: `1-projects/NOTEBOOKLM_RECON_[SOURCE_ID].md` hoặc `workspaces/learning/preview/NOTEBOOKLM_RECON_[SOURCE_ID].md`
