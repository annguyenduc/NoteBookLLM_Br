...
CHECKPOINT:
  ...
  prerequisites:
    - file: "3-resources/raw_sources/[tên_file]"
      exists: "YES | NO"
    - file: ".agent/skills/references/SOURCE_TEMPLATE.md"
      exists: "YES | NO"
    - file: ".agent/skills/references/CONCEPT_TEMPLATE.md"
      exists: "YES | NO"
...
- [ ] File nguồn đã được đặt vào `3-resources/raw_sources/` hoặc `3-resources/raw_ingest/` (R1 — chỉ User mới được thêm vào raw_*/).
...
**Đọc trước khi làm:**
- `.agent/skills/references/` — thư mục chứa các Template chuẩn (CONCEPT, ENTITY, SOURCE)
...
1. **Đọc lại file nguồn gốc** trong `raw_sources/` hoặc `raw_ingest/` — KHÔNG dùng từ Analysis file (R3).
...
3. **Tạo/Cập nhật trang Khái niệm & Thực thể**:
   - Khởi tạo các trang trong `3-resources/wiki/concepts/` và `3-resources/wiki/entities/` theo đúng form `CONCEPT_TEMPLATE.md` và `ENTITY_TEMPLATE.md`.
   - **BẮT BUỘC (R18)**: Mỗi trang phải có khối `## Ví dụ đối chiếu (R18: Double Examples)` với đúng 2 ví dụ: gốc từ sách + ứng dụng sư phạm.
...
**Sau mỗi atom được tạo — WRITE REPORT bắt buộc (R12)**:
```
WRITE REPORT:
...
- **Source Integrity**: Mọi claim trong Wiki PHẢI trích dẫn nguồn từ file raw (R3).
...
- ❌ Báo cáo "Đã tạo" mà chưa có tool call thực thi (vi phạm R2)
...
- Template Wiki: `.agent/skills/references/`
