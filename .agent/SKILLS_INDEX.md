...
## 5. 📤 Tổng Hợp & Đầu Raw / Wiki (Synthesis & Delivery)
...
- **`pedagogy`**: Chuyển kiến thức thô thành giáo án (Lesson Plan) hoặc Slide (PPTX) dựa trên mô hình sư phạm (Yêu cầu Trainer Profile).
- **`wiki-learning-pack`**: Thiết kế gói học nhanh 60-90 phút hoặc lộ trình ôn tập từ các Atom hiện có trong Vault.
- **`wiki-export-reader`**: Xuất bản tài liệu học tập của một nguồn thành HTML Reader tự chứa và sách EPUB di động theo đúng thứ tự Gói học nhanh.
...
- **`prompt-master`**: Trình biên dịch và tối ưu hóa Prompt cho các công cụ AI (như LLMs, Cursor, Antigravity, Claude Code...).

## 8. 🧠 Phương Pháp Luận Lõi (Core Methods - CM)
Nhóm kỹ năng nền tảng chuẩn hóa quy trình kỹ thuật.
- **`requesting-code-review`**: Quy trình chuẩn bị diff và yêu cầu rà soát nội bộ qua chat trước khi merge.
- **`receiving-code-review`**: Tiếp nhận feedback, cấm biểu diễn xã giao, thúc đẩy phản biện thực chứng.
...
- **`systematic-debugging`**: Deep Root Cause Analysis (RCA), chẩn đoán, viết test tái hiện và sửa lỗi tận gốc.
...
- **`test-driven-development`**: Quy trình phát triển hướng kiểm thử (Test-Driven Development) và sửa lỗi tự động.
...
- **`dispatching-parallel-agents`**: Điều phối các tác nhân song song độc lập khi đối mặt với nhiều lỗi hoặc tác vụ không phụ thuộc nhau.
