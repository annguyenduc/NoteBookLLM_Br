...
RECONCILER_SCRIPT = os.path.join(ROOT_DIR, ".agent", "skills", "wiki-absorb", "scripts", "reconciler.py")
...
def setup_db():
    ...
    # 4. Conflict High Confidence (Heuristic Resolve)
    cursor.execute("""
        INSERT INTO atoms (file_id, title, type, status, confidence, file_hash)
        VALUES (?, ?, ?, ?, ?, ?)
...
def verify():
    ...
    # 3. Test Heuristic Resolve (High Confidence)
    ...
    print(f"[Test 3] HEURISTIC RESOLVE -> {status}, Edge: {edge_count} (Expected: VERIFIED, 1) -> {'PASS' if status == 'VERIFIED' and edge_count > 0 else 'FAIL'}")
