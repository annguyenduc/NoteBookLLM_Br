 ...
 * Incorporates explicit variables, lists, and broadcast types to prevent Blockly injection errors.
 * Status: PREVIEW_ONLY
 */
...
async function loadExtension(page, displayName) {
  ...
  try {
    await page.locator(`text="${displayName}"`).first().click({ timeout: 5000 });
    ...
    // Try back
    try { await page.locator('text="Back"').first().click({ timeout: 2000 }); await page.waitForTimeout(500); } catch (e2) {}
    return false;
  }
}
...
(async () => {
  console.log('=== PRG AI Blocks v5 — Generate BT2 to BT10 (explicit variables pattern) ===');
