/**
 * generate_all_v8.js
 * Batch generator for BT1-BT10 with .sb3 and .xml export.
 * Status: PREVIEW_ONLY
 */
...
async function generateExercise(browser, ex) {
  ...
  try {
    ...
    const sb3Path = path.join(OUT_DIR, `bt${ex.id}_code.sb3`);
    await download.saveAs(sb3Path);
    console.log(`  [OK] bt${ex.id}_code.sb3 saved`);
  } catch (sb3Err) {
    console.log(`  [WARN] Failed to export SB3: ${sb3Err.message.substring(0, 150)}`);
  }
...
// ============================================================
// EXERCISES DEFINITIONS (WITH VARIABLE DECLARATIONS)
// ============================================================

const exercises = [
  // BT1: Chatbot hoc tu vung - Translate + TTS
  {
    id: 1,
    description: 'BT1 - Chatbot hoc tu vung thong minh (Translate + TTS)',
                  ...
                  <block type="operator_join" id="bt1_join">
                    ...
                    <value name="STRING2">
                      <block type="operator_join" id="bt1_join2">
                        <value name="STRING1"><block type="data_variable" id="bt1_score"><field name="VARIABLE" id="var_diem" variabletype="">Diem</field></block></value>
  ...
  <block type="poseFace_affdexWhenExpression" id="bt7_face_smile" x="520" y="350">
    <value name="EXPRESSION">
      <shadow type="poseFace_menu_EXPRESSION">
        <field name="EXPRESSION">smile</field>
      </shadow>
              ...
              <block type="operator_join" id="bt10_join1">
                ...
                <value name="STRING2">
                  <block type="operator_join" id="bt10_join2">
                    <value name="STRING1"><block type="data_variable" id="var_soco" variabletype="">SoCo</field></block></value>
...
(async () => {
  console.log('=== PRG AI Blocks v8 — Generate BT1 to BT10 and Export SB3 + XML ===');
  ...
  for (const r of results) {
    if (r.status === 'OK') {
      console.log(`  BT${r.id}: [OK] bt${r.id}_code.raw.png + bt${r.id}_code.sb3`);
      passed++;
    } else {
