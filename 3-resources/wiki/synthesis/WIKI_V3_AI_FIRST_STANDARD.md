...
- `file_id`, `title`, `type`, `status`, `tags`, `ai-first`, `confidence`, `last_reconciled`, `created`, `last_updated`.
- **Lưu ý 0 (file_id Format)**: Bắt buộc theo định dạng `TYPE_[CATEGORY/PROJECT]_[Name]` (vd: `SOURCE_AI_ThinkDataScience`, `QUERY_GOV_AuditLog`).
