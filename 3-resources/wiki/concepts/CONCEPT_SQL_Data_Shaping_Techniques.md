...
- **Cách giải quyết:** Hệ thống chấm điểm cũ xuất ra file Excel dạng Wide (Cột Toán, Cột Văn, Cột Anh). Nền tảng học tập AI thế hệ mới lại yêu cầu dữ liệu dạng Tidy/Long (Một cột "Môn_Hoc", một cột "Diem_So"). Kỹ sư dữ liệu giáo dục sử dụng `UNION ALL` để "Unpivot" 3 cột này thành 1 hàng duy nhất cho mỗi môn.
```sql
