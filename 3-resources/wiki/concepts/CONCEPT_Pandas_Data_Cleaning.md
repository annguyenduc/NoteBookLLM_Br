# CONCEPT: Pandas Data Cleaning (Làm sạch dữ liệu)
...
- **Cách giải quyết:** Khi tổng hợp danh sách đăng ký câu lạc bộ từ nhiều nguồn (Google Form, File Excel lớp), nhiều học sinh điền tên nhiều lần. Giáo viên sử dụng `drop_duplicates` trên cột Mã HS và dùng `keep='last'` để lấy thông tin đăng ký mới nhất.
```python
