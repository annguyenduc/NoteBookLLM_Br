# CONCEPT: Pandas Aggregation (Gom nhóm và Tổng hợp)
...
- **Cách giải quyết:** Thay vì phải dùng Excel PivotTable thủ công cho từng khối, giáo viên dùng Pandas để gom nhóm (`groupby`) học sinh theo Khối Lớp và Môn Học, sau đó dùng `agg` để tính ra Điểm trung bình, Điểm cao nhất và Số lượng học sinh đi thi chỉ trong 2 dòng code.
```python
